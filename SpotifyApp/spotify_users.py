...
def testitt(seconds, access_token):
...
def get_user_info(access_token):
    ...
    if response.status_code == 200:
        ...
        db_session = Session()
        existing_user = db_session.query(Users).filter_by(user_spotify_id=user_spotify_id).first()
        if existing_user:
            ...
            user = Users(user_spotify_id=user_spotify_id, name=user_name, image_url=user_image_url, playlist=playlists,
                         top_artists=top_artists,
                         top_tracks=top_tracks, following=following, currently_playing=currently_playing,
