...
from GUI import ColoredBoxLayout, StyledButton


class WidgetScreen(Screen):
    def __init__(self, **kwargs):  # Dodaj user_id jako argument
        super(WidgetScreen, self).__init__(**kwargs)
        self.orientation = 'vertical'
        self.spacing = 10
        self.size_hint_y = None

    def view(self, instance, user_id):
        layout = currently_listening(user_id)
...
def download_user_data(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM spotify_users WHERE user_spotify_id = ?', (user_id,))
...
def currently_listening(user_id):
    file = ".cache"
    ...
    def update_data(dt):
        ...
        rows = download_user_data(user_id)  # Pobierz nowe dane użytkownika
        for row in rows:
    ...
    main_layout = all_functions(main_layout, user_id)
...
def all_functions(main_layout, user_id):
    ...
    following_layout = following(user_id)
    albums_layout = albums(user_id)
    playlist_layout = playlists(user_id)  # Create playlist_layout after the loop
...
def playlists(user_id):
    ...
    rows = download_user_data(user_id)
    for row in rows:
...
def top_artists(user_id):
    ...
    for row in rows:
        ...
        for artist in top_artists:
            ...
            playlist_box.add_widget(label)
            box.add_widget(playlist_box)
    scroll_view = ScrollView(size_hint=(0.97, None), do_scroll_y=False, bar_color=[1, 0, 0, 1],
                             bar_inactive_color=[1, 0, 0, 1])
    scroll_view.add_widget(box)
    return scroll_view
